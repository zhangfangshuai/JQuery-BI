    ...
    series: [
        {
            ...
            data: [],
            silent: true,
            lineStyle: { normal: { opacity: .8, width: 1.5 } },
            ...
            effect: {
                ...
                show: true,
                animation: false,
                symbol: "image://../images/img_car.png",
                trailLength: 0,  // 行驶尾迹效果
                color: '#ffffff',
                symbolSize: 15
            },
            zlevel: 3
            ...
            coordinateSystem: 'bmap',
            symbolSize: 20,
            symbol: 'image://../images/fun_park.jpg',
            data: []
        }
    ]
};
...
var urbanOption = {
    title: {
        text: '',
        textStyle: { fontWeight:600, fontSize:17, color: '#ddd' },
        subtext: '',
        subtextStyle: { fontWeight:600, fontSize: 15, color: '#ccc' },
        top: '12%',
        left: 'center'
    },
    legend: {
        ...
        bottom: '10%',
        right: '9%',
        textStyle: { fontWeight:600, fontSize: 14, color: '#ccc' }
    },
    tooltip : {
        trigger: 'item',
        formatter: function(p) {
            return p.seriesName + '城市: ' + p.name + '<br/>开城日期: ' + p.value[3];
        }
    },
    geo: {
       map: 'china',
...
var callOption = {
    title: {
        text: '正在监听来电...',
        textStyle: { fontWeight:600, fontSize:17, color: '#bbb' },
        top: '10%',
        left: 'center'
    },
    tooltip : {
        trigger: 'item',
        formatter: function(p) {
            return '城市: ' + p.name + '<br/>呼叫数: ' + p.value[2];
        }
    },
    geo: {
       map: 'china',
...
var topOption = {
    ...
    xAxis: { type: 'value', axisLabel: {color:'#8898ad', rotate: 90} },
    yAxis: { type: 'category', axisLabel: {color:'#8898ad'}, data: [] },
    series: [ {
        name:'来电数',
        type:'bar',
        markPoint: {
            symbolSize: 45,
            data: [ {type: 'max', name: '最大值', itemStyle: {normal: {color:'#0ec774'}} } ]
        },
        markLine: {
            data: [ {type : 'average', name:'平均值', lineStyle:{normal:{width:2, color: '#3bffbc' }} } ]
        },
        data: []
    }]
}
...
var parksOption = {
    tooltip: {
        ...
        position: function (point, params, dom, rect, size) {
           var vx = size.viewSize[0], vy = size.viewSize[1];
           var cx = size.contentSize[0], cy = size.contentSize[1];
           var px = point[0], py = point[1];
           var x = (vx - cx - px) > 30 ? (px + 20) :
                   (px - cx) > 30 ? (px - cx - 20) :
                   px < (vx/2) ? ((vx-cx)/2 + 10) : ((vx - cx)/2 - 10);
           var y = py < (vy/2) ? (py + 30) : (py - cy - 20);
        ...
        formatter: function (p) {
            if (p.data.hasOwnProperty('fromName')) {
                return '流出网点: ' + (p.data.fromName || '') +'<br/>流入网点: '+ (p.data.toName || '') +'<br/>订单数: ' + (p.data.orderNum || '');
            }
            return '类型: ' + (p.seriesName || '') +'<br/>地址: '+ (p.name || '') +'<br/>车位数: '+ (p.data.carportNum || '') +
                   '<br/>车位均单: ' + (p.data.carportAvgorder || '') + '<br/>取车订单: ' + (p.data.orderNum || '');
        }
    },
    bmap: {
        center: [116.46, 39.92],
    ...
    series: [
        {
            ...
            coordinateSystem: 'bmap',
            symbol: ['none', 'arrow'],
            lineStyle: { normal: { opacity: .8, width: 2.5, curveness: 0.3, color: '#7575ec' } },
            effect: { show: true, constantSpeed: 25, trailLength: 0, symbolSize: 18,
                symbol: 'image://../images/img_car.png'
            },
            data: []
        },{
            ...
            coordinateSystem: 'bmap',
            symbol: ['none', 'arrow'],
            lineStyle: { normal: { opacity: .8, width: 2.5, curveness: 0.3, color: '#ff3ee8' } },
            effect: { show: true, constantSpeed: 25, trailLength: 0, symbolSize: 18,
                symbol: 'image://../images/img_car.png'
            },
            data: []
